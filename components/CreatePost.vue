  ...
  <div id="CreatePost" class="fixed z-50 bottom-0 h-full w-full overflow-hidden">
    <div class="bg-black h-full w-full text-white overflow-auto">
      ...
      <button
        ...
        :disabled="isLoading"
        @click="createPost"
        class="fixed bottom-0 font-bold text-lg w-full p-2 bg-black inline-block float-right p-4 border-t border-t-gray-70"
...
const createPost = async () => {
  ...
  if (fileData.value) {
    const { data, error } = await postStore.uploadPostFile(fileData.value)
  ...
  try {
    ...
    const created = await postStore.createPost(data)
    if (created) {
      userStore.isMenuOverlay = false
      clearData()
    }
