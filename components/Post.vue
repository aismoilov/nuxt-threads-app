...
const deletePost = async (id, picture) => {
  ...
  try {
    ...
    const deleted = await postStore.deletePost(id, picture)
    if (deleted) {
      emit('isDeleted', true)
    }
...
const unlikePost = async ({ id, postId }) => {
  ...
  await postStore.unlikePost(data)
  isLike.value = false
}
...
const likesFunc = () => {
  ...
  if (likePostObj) {
    unlikePost({ id: likePostObj.id, postId: likePostObj.postId })
    return null
  }
